...
export default function Curtain() {
  ...
  const calculateResult = () : void => {
    ...
    if (cuts === 0 && multiple === 0 && amount === 0 && length === 0 && width === 0 && fabricWidth === 0 || !cutType && !noCutType
      ...
      let fabricWidthAmount = getFabricWidthAmount(width, fabricWidth, amount);
      let yards = fabricWidthAmount * length / 36;
      let meters = yards * 36 / 39;
      setResult(`${(yards * 1.03 + 0.1).toFixed(1)}y\n${(meters * 1.03 + 0.1).toFixed(1)}m`)
    }
  }

  const getFabricWidthAmount = (width:number, fabricWidth:number, amount:number) : number => {
    var fabricWidthAmount:number;
    if (cuts === 0) {
      fabricWidthAmount = (width * multiple + 5) / fabricWidth;
    } else {
      fabricWidthAmount = (width * multiple + 10) / fabricWidth;
    }
    return roundFabricWidthAmount(amount, fabricWidthAmount);
  }
